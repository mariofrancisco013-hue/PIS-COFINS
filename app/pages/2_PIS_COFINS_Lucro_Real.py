...
with aba_apuracao:
    ...
    if not linhas_salvas:
        ...
        df_ap = pd.DataFrame(linhas_salvas)
        df_ap["situação"] = df_ap["manual"].map(lambda m: "⏳ manual/pendente" if m else "calculado")
        st.dataframe(
            df_ap, use_container_width=True, hide_index=True,
            column_order=["linha", "descricao", "valor_pis", "valor_cofins", "situação"],
            column_config={
                "valor_pis": coluna_moeda("PIS"), "valor_cofins": coluna_moeda("COFINS"),
            },
        )
        ...
        n_pendentes_manual = int(df_ap["manual"].sum())
